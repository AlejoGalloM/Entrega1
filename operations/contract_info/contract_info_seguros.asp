...
write_dataLog Response.Status,"contract_info_seguros.asp","process_selection Loaded by: " & Session("sp_miLogin") & " selected  for the Contract " & Contract,ClientId,"ContratoRetirosPAC_GetForContrato " & Contract & " - ContratoRetirosPAC_GetForContrato " & Contract,"N/A","null","Consulta","N/A"
...
'=========================================================================================
'=Rubén Ballesteros 2015-12-17 - Consulta WS OM.Contracts para obtener el contrato en ULLA
'=========================================================================================

'=======================================================
'START FUNCTIONS DEFINITION
'=======================================================
Function NodeValue(nodes, value)
	NodeValue = ""
    Dim xNode
    ' Se recorren los nodos tipos NODE_ELEMENT
    For Each xNode In nodes 
		If xNode.nodeName = value Then
...
'=======================================================
Dim contract_xmlDoc, contract_HTTP, otroContract, nodes
		
Set contract_HTTP = CreateObject("Microsoft.XMLHTTP")
Set contract_xmlDoc = CreateObject("MSXML.DOMDocument")
contract_xmlDoc.Async = False

Session.Contents("SecondaryContract") = 0


If Trim(Product) = "OMPEV" Then
    ' Invoca el ws de Contratos
	contract_HTTP.Open "GET", Application("WSContracts") & "/GetDetail/GetContractDetail?number=" & cstr(Contract) & "&productCode=" & Trim(Product) & "&planCode=" & Plan, False
    ' "/GetDetail/GetContractDetail?number=7781&productCode=MFUND&planCode=CAH1"
	contract_HTTP.Send(null)

    If contract_xmlDoc.load(contract_HTTP.responseXML) Then
        Set nodes = contract_xmlDoc.documentElement.childNodes
        Session.Contents("SecondaryContract") = NodeValue(nodes, "SecondaryNumber")
	End If
End If
'===========================
'Fin Consumo WS OM.Contracts
'===========================
...
'==========================================================================
				...
				OpenTr ""
					OpenTd "balance",""
						Response.Write "Valor asegurado"
					CloseTd
					OpenTd "balance", ""
						Response.Write ":"
					CloseTd										
					OpenTd "balance",""
						Response.Write FormatCurrency(arrContract(106,0),2)
						'Response.Write (arrContract(106,0)
					CloseTd
					OpenTd "balance",""
						Response.Write "&nbsp;"
					CloseTd
...
OpenForm "menu", "post", "../menu/menu.asp", "target=menu"
    ...
    PlaceInput "SecondaryContract", "hidden", Session("SecondaryContract"), "" 
CloseForm
%>
<script language=javascript>
